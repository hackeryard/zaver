...
int zv_http_parse_request_line(zv_http_request_t *r) {
    ...
    for (pi = r->pos; pi < r->last; pi++) {
        ...
        switch (state) { // using state machine

        /* HTTP methods: GET, HEAD, POST */
        case sw_start:
            r->request_start = p;

            if (ch == CR || ch == LF) { // 何时出现CRLF的情况
                break;
            }
        ...
        case sw_method:
            if (ch == ' ') {
                ...
                // 在sw_method状态下，遇到' '，说明可以解析method而进入下一个状态
                r->method_end = p;
                m = r->request_start;
        ...
        case sw_major_digit:
            ...
            r->http_major = r->http_major * 10 + ch - '0'; // why
            break;
